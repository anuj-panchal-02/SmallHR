...
public class MetricsAdminController : ControllerBase
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<MetricsAdminController> _logger;

    public MetricsAdminController(
        ApplicationDbContext context,
        ILogger<MetricsAdminController> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    /// </summary>
    [HttpGet("overview")]
    public async Task<IActionResult> GetOverview([FromQuery] DateTime? startDate = null, [FromQuery] DateTime? endDate = null)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Error fetching metrics overview");
            return StatusCode(500, new { message = "Error fetching metrics", error = ex.Message });
        }
    }
    ...
    public async Task<IActionResult> GetRevenueTrends([FromQuery] int months = 12)
    {
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { message = "Error fetching revenue trends", error = ex.Message });
        }
    }
    ...
    public async Task<IActionResult> GetTenantGrowth([FromQuery] int months = 12)
    {
        try
        {
            var startDate = DateTime.UtcNow.AddMonths(-months);
        ...
        {
            _logger.LogError(ex, "Error fetching tenant growth");
            return StatusCode(500, new { message = "Error fetching tenant growth", error = ex.Message });
        }
    }
    ...
    public async Task<IActionResult> GetChurnAnalysis([FromQuery] int months = 12)
    {
        try
        {
            var startDate = DateTime.UtcNow.AddMonths(-months);
        ...
        {
            _logger.LogError(ex, "Error fetching churn analysis");
            return StatusCode(500, new { message = "Error fetching churn analysis", error = ex.Message });
        }
    }
}
